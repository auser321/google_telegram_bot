	...
	def message_handlers(self):
		...
		def privateMessageHandler(message):
			...
			if command == "/feedback":
				...
				self.bot.reply_to(message, f"Welcome to Google-Search {message.from_user.username}")
			else:
				getMessage(message)
		...
		def getMessage(message):
			...
			if user_id in feedbacK_list and message.chat.type == "private":
				...
				self.bot.send_animation(chat_id=message.chat.id,
										animation=open(r"D:\Learning\Codes\Telegram\gifs\thanks.gif", "rb"))
			else:
				temp_response = self.bot.send_animation(chat_id=message.chat.id,
														animation=open(r"D:\Learning\Codes\Telegram\gifs\loading.gif",
																	   "rb"))
				msg = message.text
